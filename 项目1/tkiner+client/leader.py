...
from examination_and_approval_leave import *
from exam_application_for_reimbursement import *
from worker_work import *
from PIL import ImageTk, Image
def show_leader_menu(usr_name):
    root = Tk()
    ...
    work_son_menu = Menu(filemenu,tearoff=0)
    ...
    filemenu.add_cascade(label="考勤", menu=work_son_menu)
    work_son_menu.add_command(label="查看自己考勤", command=showcheck_work)
    work_son_menu.add_command(label="查看员工考勤", command=showworker_work)
    ...
    funcmenu.add_command(label="审批请假", command=showexamination_and_approval_leave)
    funcmenu.add_command(label="审批报销", command=exam_application_for_reimbursement)
    menubar.add_cascade(label="审批", menu=funcmenu)
